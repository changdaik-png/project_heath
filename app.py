...
st.set_page_config(
    page_title="2026 40대 타겟 의료비 위험 예측 및 정책 대시보드",
    page_icon="🏥",
    layout="wide"
)
...
st.markdown("""
    ...
    .main-title {
        font-size: 2.5rem;
        font-weight: 800;
        background: linear-gradient(135deg, #FF4B4B, #4B83FF);
...
# 3. 헤더 영역
st.markdown("<h1 class='main-title'>🏥 2026 40대 타겟 의료비 위험 예측 및 정책 대시보드</h1>", unsafe_allow_html=True)
st.markdown("<p class='sub-title'>한국의료패널(KHP) 데이터를 이용해 40대 장년층의 5년 뒤 의료비 변동을 시뮬레이션하고 맞춤형 공공 정책을 제안합니다.</p>", unsafe_allow_html=True)

# 4. 사이드바 - 개별 환자 입력 데이터 폼 구축 (40대 정책 타겟 일치 유도: 40세~49세 범위 제한)
...
if model_pipeline is None:
    ...
    st.markdown("<h2 class='section-header'>📂 실무자용 대량 40대 환자 데이터 병합 분석 (e_hh & e_ind 연계)</h2>", unsafe_allow_html=True)
    st.markdown("한국의료패널(KHP)의 **가구 데이터(e_hh)**와 **개인 데이터(e_ind)**를 각각 업로드해 주십시오. 시스템이 가구 식별코드(`HHID`)를 활용해 두 파일을 자동 조인(Join)하여 **만 40세~49세 대상자만 선별**한 뒤 일괄 위험 예측을 수행합니다.")
    ...
    if hh_file is not None and ind_file is not None:
        ...
        try:
            ...
            if join_key is None:
                ...
                st.info(f"🧬 두 데이터셋을 식별키 `{join_key}` 기준으로 병합 중...")
                merged_df = pd.merge(ind_df, hh_df, on=join_key, suffixes=('_ind', '_hh'))
                st.success(f"병합 완료! 총 {len(merged_df):,}건의 가구원 매칭 정보가 생성되었습니다.")
                ...
                if income_col is not None:
                    ...
                    print(f"-> [매핑 성공] 소득 컬럼은 '{income_col}'로 선택되었습니다.")
                else:
                ...
                if has_valid_age:
                    ...
                    print(f"-> [매핑 성공] 나이 컬럼은 '{age_col}'로 선택되었습니다.")
                else:
                    ...
                    if has_valid_birth:
                        ...
                        print(f"-> [매핑 성공] 생년 컬럼 '{birth_col}'을 기반으로 나이를 역산했습니다 (기준년도: {target_year}년).")
                    else:
                        np.random.seed(42)
                ...
                if gender_col is not None:
                    ...
                    print(f"-> [매핑 성공] 성별 컬럼은 '{gender_col}'로 선택되었습니다.")
                else:
                ...
                if chronic_col is not None:
                    ...
                    print(f"-> [매핑 성공] 만성질환 컬럼은 '{chronic_col}'로 선택되었습니다.")
                else:
                    ...
                    if cd_cols:
                        ...
                        print(f"-> [매핑 성공] 상세 질병코드군({len(cd_cols)}개 컬럼)을 취합하여 만성질환 여부를 생성했습니다.")
                    else:
                        ...
                        if c_cols:
                            ...
                            print(f"-> [매핑 성공] 질병코드군({len(c_cols)}개 컬럼)을 취합하여 만성질환 여부를 생성했습니다.")
                        else:
                ...
                def match_action_plan(row):
                    ...
                    if inc_val <= 500.0:  # 소득 '하'
                        return "🩺 [Track 2] 방문 진료 및 의료 바우처 지급 (사각지대 해소)"
                    elif dy > 0:  # 소득 '상/중'이면서 의료비 급증
                ...
                st.markdown(f"### 📈 일괄 조인/예측 및 정책 조치 분류 명단 (위험군 상위 10% 집중 관리)")
